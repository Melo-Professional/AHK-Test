...
class AutoUpdater {
    ...
    ApplyUpdate(silent := false) {
        ...
        ; Command to launch updated app with verification signal flag
        relaunchCmd := A_IsCompiled 
            ? '"' . newTargetPath . '" "--signal-update-success=' . signalFile . '"' 
            : '"' . A_AhkPath . '" "' . newTargetPath . '" "--signal-update-success=' . signalFile . '"'

        ; Command to relaunch OLD app in case of rollback
        restoreRelaunchCmd := A_IsCompiled 
            ? '"' . targetFile . '"' 
            : '"' . A_AhkPath . '" "' . targetFile . '"'

        cmdLines := [
            "@echo off",
            "timeout /t 1 /nobreak > nul",
            
            "; --- STEP 1: Wait for current process to exit ---",
            ":wait_exit",
        ...
        if isZip {
            cmdLines.Push('ren "' . targetFile . '" "' . backupFileName . '"')
        ...
        ; --- STEP 2: Launch updated version and check health ---
        ...
        ; --- STEP 3: ROLLBACK (If health signal wasn't received within 8 seconds) ---
        cmdLines.Push(':update_failed')
        cmdLines.Push('if exist "' . newTargetPath . '" del /f /q "' . newTargetPath . '"')
        ...
        cmdLines.Push('start "" ' . restoreRelaunchCmd)
    ...
    ShowGUI() {
        ...
        updGui := Gui("-MinimizeBox", "Update Settings - " . A_ScriptName)
        updGui.MarginX := 24
        updGui.MarginY := 20
        updGui.SetFont("s10", "Segoe UI")

        ; Status Banner Message
        if hasUpdate {
            updGui.SetFont("s11 bold", "Segoe UI")
            updGui.AddText("w320 c0x008000", "A new update is available!")
            updGui.SetFont("s9 Norm", "Segoe UI")
            updGui.AddText("w320 c0x555555 y+4", "Version " . this.LatestVersion . " is ready to install.")
        } else if (this.LatestVersion != "") {
            updGui.SetFont("s11 bold", "Segoe UI")
            updGui.AddText("w320 c0x2B579A", "✓ You're up to date")
            updGui.SetFont("s9 Norm", "Segoe UI")
            updGui.AddText("w320 c0x555555 y+4", "You are running the latest version.")
        } else {
            updGui.SetFont("s11 bold", "Segoe UI")
            updGui.AddText("w320 c0x333333", "Update Preferences")
            updGui.SetFont("s9 Norm", "Segoe UI")
            updGui.AddText("w320 c0x555555 y+4", "Check and manage application updates.")
        }

        updGui.AddText("w320 h1 0x10 y+16") ; Divider Line

        ; Version Info Grid
        updGui.SetFont("s9 Norm", "Segoe UI")
        
        updGui.AddText("w120 y+16 c0x666666", "Current Version:")
        updGui.SetFont("s9 bold")
        updGui.AddText("x+10 w180 c0x222222", this.App.Version)

        updGui.SetFont("s9 Norm")
        updGui.AddText("x24 y+10 w120 c0x666666", "Latest Version:")
        updGui.SetFont("s9 bold")
        lblLatest := updGui.AddText("x+10 w180 c0x222222", this.LatestVersion != "" ? this.LatestVersion : "Not checked")

        updGui.SetFont("s9 Norm")
        updGui.AddText("x24 y+10 w120 c0x666666", "Last Checked:")
        lblLastCheck := updGui.AddText("x+10 w180 c0x222222", this.App.UpdateLastCheck)

        updGui.AddText("x24 w320 h1 0x10 y+16") ; Divider Line

        ; Settings Section
        chkAuto := updGui.AddCheckbox("x24 y+16 Checked" . (this.App.UpdateAuto ? "1" : "0"), " Enable Automatic Updates")
        
        lblFreq := updGui.AddText("x24 y+14 c0x444444", "Check frequency (days):")
        numFreq := updGui.AddEdit("x+12 w60 Number Center", this.App.UpdateFrequencyDays)
        updUpDown := updGui.AddUpDown("Range1-90", this.App.UpdateFrequencyDays)
        ...
        updGui.AddText("x24 w320 h1 0x10 y+20") ; Divider Line

        ; Action Buttons
        btnCheck := updGui.AddButton("x24 y+16 w155 h32", "Check for Updates")
        btnUpdate := updGui.AddButton("x+10 w155 h32 " . (hasUpdate ? "" : "Disabled"), "Install Update")

        btnSave := updGui.AddButton("x24 y+12 w320 h34", "Save & Close")
