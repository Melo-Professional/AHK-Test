...
class AutoUpdater {
    ...
    CheckOnStartup(autoApplySilent := false) {
        ...
        if (diffDays >= this.App.UpdateFrequencyDays) {
            SetTimer(() => this.CheckForUpdates(true, autoApplySilent), -100)
        }
    }

    CheckForUpdates(silent := false, autoApply := false) {
        ...
        try {
            ...
            if (this.IsNewerVersion(this.App.Version, this.LatestVersion)) {
                if autoApply {
                    ; Automatically download, extract, replace & restart silently
    ...
    ShowGUI() {
        ...
        btnCheck.OnEvent("Click", (*) => (
            ...
            hasUpdate := this.CheckForUpdates(false, false),
