...
class AutoUpdater {
    ...
    ShowGUI() {
        ...
        ;MyGui := Gui("+AlwaysOnTop -MinimizeBox", "Update Settings - " . A_ScriptName)

/* 
        MyGui := Gui("-MinimizeBox", "Update Settings - " . A_ScriptName)
...
;		}
        ...
        if hasUpdate {
            ...
            abc := MyGui.AddText("vBypassTheme_01 xm w320 c0x008000", "A new update is available!")
            MyGui.SetFont("s9 Norm", "Segoe UI")
            MyGui.AddText("vSmooth_02 xm w320 c0x555555 y+4", "Version " . this.LatestVersion . " is ready to install.")
        } else if (this.LatestVersion != "") {
            MyGui.SetFont("s11 bold", "Segoe UI")
            MyGui.AddText("vBypassTheme_03 xm w320 c0x2B579A", "✓ You're up to date")
            MyGui.SetFont("s9 Norm", "Segoe UI")
            MyGui.AddText("vSmooth_04 xm w320 c0x555555 y+4", "You are running the latest version.")
        } else {
            MyGui.SetFont("s11 bold", "Segoe UI")
            MyGui.AddText("xm w320 c0x333333", "Update Preferences")
            MyGui.SetFont("s9 Norm", "Segoe UI")
            MyGui.AddText("xm w320 c0x555555 y+4", "Check and manage application updates.")
        }

        MyGui.AddText("w320 h1 0x10 y+16") ; Divider Line

        ; Version Info Grid
        MyGui.SetFont("s9 Norm", "Segoe UI")
        
        MyGui.AddText("w120 y+16 c0x666666", "Current Version:")
        MyGui.SetFont("s9 bold")
        MyGui.AddText("vStrong_01 x+10 w180 c0x222222", this.App.Version)

        MyGui.SetFont("s9 Norm")
        MyGui.AddText("x24 y+10 w120 c0x666666", "Latest Version:")
        MyGui.SetFont("s9 bold")
        lblLatest := MyGui.AddText("vStrong_02 x+10 w180 c0x222222", this.LatestVersion != "" ? this.LatestVersion : "Not checked")

        MyGui.SetFont("s9 Norm")
        MyGui.AddText("x24 y+10 w120 c0x666666", "Last Checked:")
        lblLastCheck := MyGui.AddText("x+10 w180 c0x222222", this.App.UpdateLastCheck)

        MyGui.AddText("x24 w320 h1 0x10 y+16") ; Divider Line

        ; Settings Section
        chkAuto := MyGui.AddCheckbox("x24 y+16 Checked" . (this.App.UpdateAuto ? "1" : "0"), " Enable Automatic Updates")
        
        lblFreq := MyGui.AddText("x24 y+14 c0x444444", "Check frequency (days):")
        ...
        MyGui.AddText("x24 w320 h1 0x10 y+20") ; Divider Line

        ; Action Buttons
        btnCheck := MyGui.AddButton("x24 y+16 w155 h32", "Check for Updates")
        btnUpdate := MyGui.AddButton("x+10 w155 h32 " . (hasUpdate ? "" : "Disabled"), "Install Update")

        btnSave := MyGui.AddButton("x24 y+12 w320 h34", "Save & Close")
